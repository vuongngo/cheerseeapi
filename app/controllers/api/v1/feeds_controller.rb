class Api::V1::FeedsController < ApplicationController
  ...
  def show
    user = User.find(params[:id])
  	contest = Contest.order_by(:created_at.desc).where('u.u_id' => params[:id])
  	participation = Participation.order_by(:created_at.desc).where('u.u_id' => params[:id])
  	activity = contest + participation
    activity.sort! { |a, b| b.created_at <=> a.created_at  }
    activity = Kaminari.paginate_array(activity).page(params[:page]).per(params[:per_page])
  ...
  def association
  	...
  	participations = Participation.order_by(:created_at.desc).where(contest_id: params[:id]).page(params[:page]).per(params[:per_page]).limit(50)
